...
class PointSphereApp : public AppNative
{
public:
	void setup() override;
	...
	struct SpherePoint
	{
		...
		vec3 PNormal;
		vec2 PTexCoord;
		SpherePoint(vec3 pPos, vec3 pNorm, vec2 pUV) : PPosition(pPos), PNormal(pNorm), PTexCoord(pUV){}
	};
...
void PointSphereApp::setup()
{
	...
	getSignalShutdown().connect(std::bind(&PointSphereApp::stopDS, this));
}

void PointSphereApp::setupMesh()
{
	try
	{
		mShaderObj = gl::GlslProg::create(loadAsset("pointsphere_vert.glsl"), loadAsset("pointsphere_frag.glsl"));
	}
	...
	vector<SpherePoint> cPoints;
	for (int vy = 0; vy < S_DIMS.y; vy++)
	{
		for (int vx = 0; vx < S_DIMS.x; vx++)
		{
			float cy = lmap<float>(vy, 0, S_DIMS.y, 1.0f, -1.0f);
			// vx * ((180.0f/(float)S_DIMS.x)+90.0f)
			//float cx = math<float>::sin(toRadians(vx * ((180.0f / (float)S_DIMS.x) + 90.0f)));
			//float cz = math<float>::cos(toRadians(vx * ((180.0f / (float)S_DIMS.x) + 90.0f)));
			float cx = lmap<float>(vx, 0, S_DIMS.x, 1.3333f, -1.3333f);
			float cz = 0.0;

			cPoints.push_back(SpherePoint(vec3(cx, cy, cz),vec3(0, 0, 0),vec2(1.0-vx/(float)S_DIMS.x, 1.0f-vy/(float)S_DIMS.y)));
		}
	}
	...
	mAttribObj.append(geom::POSITION, 3, sizeof(SpherePoint), offsetof(SpherePoint, PPosition));
	mAttribObj.append(geom::NORMAL, 3, sizeof(SpherePoint), offsetof(SpherePoint, PNormal));
	mAttribObj.append(geom::TEX_COORD_0, 2, sizeof(SpherePoint), offsetof(SpherePoint, PTexCoord));
...
void PointSphereApp::setupDSAPI()
{
...
void PointSphereApp::mouseDown( MouseEvent event )
{
	mMayaCam.mouseDown(event.getPos());
}

void PointSphereApp::mouseDrag(MouseEvent event)
{
	mMayaCam.mouseDrag(event.getPos(), event.isLeftDown(), false, event.isRightDown());
}

void PointSphereApp::update()
{
	if (mCinderDS->update())
	{
...
void PointSphereApp::stopDS()
{
	mCinderDS->stop();
}

CINDER_APP_NATIVE( PointSphereApp, RendererGl )
